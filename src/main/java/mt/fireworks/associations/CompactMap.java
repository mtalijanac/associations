...
public class CompactMap<T> {
    ...
    int closedSegmentIndex;
    
    @NonNull SerDes<T> serdes;
    Function<T, byte[]> keyer;
    MutableObjectLongMap<byte[]> index;
    
    float segmentWasteThreshold = 0.9f;
    long  segmentSizeThresholdInBytes = 10 * 1024 * 1024; // 10 Mb
    ...
    public CompactMap(int segmentCount, SerDes<T> serdes, Function<T, byte[]> keyer) {
        ...
        this.index = ObjectLongHashingStrategyMaps.mutable.of(new BytesHashingStrategy());
    }
    ...
    public boolean add(T value) {
        ...
        long pointer = this.index.updateValue(key, -1l, currentPointer -> {
            if (currentPointer == -1l)
                return addToRandomList(value);
            
            int segementIndex = segementIndex(currentPointer);
            if (segementIndex == this.closedSegmentIndex)
                return addToRandomList(value);
            
            return addToSegment(segementIndex, value);
        });
        return true;
    }
    
    
    long addToRandomList(T value) {
        ThreadLocalRandom rng = ThreadLocalRandom.current();
        int segmentIdx;
        do segmentIdx = rng.nextInt(segments.length);
        while (closedSegmentIndex == segmentIdx);
        return addToSegment(segmentIdx, value);
    }
    
    long addToSegment(int segmentIndex, T value) {
    ...
    void compactSegment(final int oldSegmentIndex) {
        ...
        oldSegment.forEach((position, bucket, off, len) -> {
            ...
            long newPointer = addToSegment(newSegmentIndex, value);
    ...
    static class CompactMapMetrics implements Metrics {
        ...
        public String text(boolean coments) {
            ...
            sb.append("  segmentsCompactedCount: ").append(segmentsCompactedCount).append("\n");
            
            sb.append("  liveDataCopied: ").append(liveDataCopied).append(" bytes \n");
            sb.append("  deadDataReleased: ").append(deadDataReleased).append(" bytes \n");
